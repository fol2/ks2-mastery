...
5. **`createGrammarQuestion` call-count ceiling.** Instrumented test pins maximum invocations per queue build. Post-R6 ceiling reflects memoised regime (≥5× reduction vs pre-R6 count). Regression raises CI failure.
