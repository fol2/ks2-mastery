...
| 30-learner classroom beta | Certification candidate — P3 T1, T5 repeat 1, and T5 repeat 2 all passed the strict 30 learner gate with joined invocation CPU/wall telemetry. Public/Admin promotion is intentionally pending a separate capacity-status row PR. | Add a reviewed evidence-table row for the P3 terminal run, regenerate `reports/capacity/latest-evidence-summary.json`, and keep the separate status change tied to the P3 completion report. |
...
| --- | --- | --- | --- | --: | --: | --: | --: | --: | --: | --: | --- | --- | --- |
| 2026-04-30 | 3af2b44 | production | 30-learner-beta P3-T5 strict repeat 2 | 30 | 20 | 1 | 715.2 | 279.7 | 29597 | 0 | none | 30-learner-beta-certified | [json](reports/capacity/evidence/2026-04-30-p3-t5-strict-r2.json) |
...
> **P3 strict repeat 2 promoted (2026-04-30, 30-learner-beta-certified).** P3-T5 repeat 2 is the reviewed terminal evidence row. Bootstrap P95 715.2 ms (vs 1,000 ms ceiling, 28.5% headroom), command P95 279.7 ms (vs 750 ms ceiling, 62.7% headroom), max bytes 29,597 B, zero 5xx, zero network failures, zero capacity signals. Invocation telemetry: 10/10 retained top-tail CPU/wall coverage, 10/10 statement-log coverage, 0 join warnings. Promoted via reviewed capacity-status PR. Public claim moves from `small-pilot-provisional` to `30-learner-beta-certified`. 60+ learners remain unclaimed.
...
> **P3 strict evidence (2026-04-30, certification candidate).** P3 repaired the missing invocation telemetry path and captured three strict production 30 learner runs with `--demo-sessions`, `--learners 30`, `--bootstrap-burst 20`, `--rounds 1`, and `--config reports/capacity/configs/30-learner-beta.json`. P3-T1 passed with bootstrap P95 701.3 ms and command P95 292.7 ms; P3-T5 repeat 1 passed with bootstrap P95 661.4 ms and command P95 319.2 ms; P3-T5 repeat 2 passed with bootstrap P95 715.2 ms and command P95 279.7 ms. Each joined 10/10 retained bootstrap top-tail samples for invocation CPU/wall and 10/10 for statement logs with zero join warnings. These files are committed as candidate evidence, but the Admin/latest summary remains fail-closed as `evidence-not-in-verified-capacity-table` until a separate reviewed capacity-status row promotes the public status.
...
- The smoke creates (or reuses) a demo session, loads `/api/bootstrap`, starts a Smart Review spelling session with `mode: 'smart'`, submits one deliberately-wrong answer, then runs a fixed-slug recovery round (`single` / `possess`: wrong → immediately correct) so the human retry path is covered under the same learner profile. It captures the client-observed wall time for the Smart Review `start-session` command (latency gate) and fails on any 5xx / `exceededCpu` on the recovery submits.
- Local unit coverage for the same human path (dense history, concurrent CAS, monkey retype bursts) lives in `tests/worker-spelling-wrong-then-correct-capacity.test.js`. Classroom load (`scripts/classroom-load-test.mjs`) still does not monkey spelling wrong→correct; use the dense smoke + that unit suite until a production monkey driver exists.
- Spelling commands write a **compact public projection** (`public_ui_json`) atomically with authority state. The projection is bootstrap-sized (no feedback/analytics/audio word groups). Command `subjectReadModel` remains richer for the active UI. Structured logs may include allowlisted `commandPhaseTimings` (`preflight`, `content`, `workingSet`, `engineApply`, `rewardProjection`, `readModelBuild`, `d1Batch`); those timings never appear on child-facing `meta.capacity`.
