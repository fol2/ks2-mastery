...
| `worker/src/repository.js:4300` (`updateManagedAccountRole`) | **H3 (Phase C reviewer) upgrade**: removed, now composes UPDATE + mutation receipt in a single `batch()`; CAS guard combines `repo_revision = ?` with the existing last-admin subquery. A rowsAffected=0 outcome disambiguates into 409 `account_role_stale` vs 409 `last_admin_required` via a follow-up SELECT. | CAS guard via `repo_revision` + last-admin subquery; UPDATE + receipt in single `batch()`. | `tests/worker-admin-ops-mutations.test.js` admin-role subset + H3 additions (stale CAS, atomicity, last-admin regression) green |
