...
- Edge case `BUILD_HASH` absent: server has no `__BUILD_HASH__` / `env.BUILD_HASH` → `resolved_in_release = 'unknown'`; error event resolves cleanly; auto-reopen rule (U17) will NOT fire against `'unknown'` because the rule additionally requires `resolved_in_release` matches a SHA-shaped regex.
