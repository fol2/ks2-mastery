...
PR #699, `feat(capacity): harden P3 worker telemetry gate`, completed the telemetry-tooling SDLC loop and was merged to `main` on 2026-04-30 as squash merge result commit `32cd5a306f62245cbfb81893934a7aab6e5fc062`.
...
| Synthetic fixture boundary | The P3 canonical fixture uses fixture-only request IDs rather than raw-looking production request IDs. Legacy fixtures may still exercise raw-ID redaction behaviour. |
...
| The execution plan frontmatter overstated completion scope. | The plan now says `status: partially-executed`, marks the source contract open, and explicitly states that production strict evidence, classification, and final P3 decision remain open. |
| "Redacted fixture" wording conflicted with a synthetic fixture containing raw-looking IDs. | The P3 canonical fixture now uses fixture-only request IDs, and the plan distinguishes committed production-derived evidence artefacts from legacy parser fixtures used only for redaction regression coverage. |
...
A post-merge gap review found that the wording still risked overclaiming original P3 completion. This report now uses `tooling-complete` for the merged #699 slice and leaves the original P3 contract open. The same gap review also identified two concrete follow-up fixes: timestamp-free log exports now produce `capture-window-missing-log-timestamps`, and the P3 canonical fixture no longer contains raw-looking `ks2_req_*` request IDs.
...
Remote PR CI on the rebased PR head `97ef43c56d968692331c7083ab99835e96952cd5` passed before merge. This is PR-head evidence, not a separate claim that the squash merge result commit had its own workflow run:
...
- The P3 canonical parser fixture uses fixture-only request IDs.
- Legacy parser fixtures may still contain raw-looking IDs only where they explicitly exercise redaction compatibility.
- New production-derived committed P3 artefacts must not contain raw `ks2_req_*`, SQL text, table or column names, cookies, bearer tokens, learner names, answers, request bodies, or response bodies.

This keeps the repo useful for deterministic tests while preserving the fail-closed redaction stance for live evidence. It does not assert that every historical evidence file in the repository is already free of raw request IDs; older committed capacity evidence still needs separate residual-risk handling.
...
- The log export has no usable timestamps, so capture-window overlap cannot be proven.
...
| Original P3 contract | Open; not completed by PR #699 or PR #705 |
| Required final P3 report | Not yet created: `docs/plans/james/sys-hardening/A/sys-hardening-optimisation-p3-completion-report.md` |
