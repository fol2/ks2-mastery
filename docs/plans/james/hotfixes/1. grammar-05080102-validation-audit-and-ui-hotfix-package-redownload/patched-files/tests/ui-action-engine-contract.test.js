...
test('grammar session disables End round while runtime is read-only or pending', () => {
  ...
  assert.match(
    source,
    /<Button[\s\S]*variant="ghost"[\s\S]*disabled=\{runtimeReadOnly \|\| pending\}[\s\S]*actions\.dispatch\('grammar-end-early'\)[\s\S]*>\s*End round\s*<\/Button>/,
    'Normal-session End round must be disabled in read-only runtime state and while pending.',
  );
});
