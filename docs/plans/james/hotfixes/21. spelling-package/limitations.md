...
- The secure-vocabulary source list was originally approved by James for import/reviewer-pack generation only. James later approved secure-extension import and generated release-quality fallback fields for all pinned candidate rows; that approval is recorded in `evidence/secure-extension-import-approval-record-2026-05-17.md` and `.json`, then ingested through `evidence/secure-extension-import-approval-pipeline-record-2026-05-17.json`.
- The approved secure-vocabulary source is now imported locally into `content/spelling.seed.json` and generated runtime data as release `spelling-r6`. This writes local repo content only; it does not prove D1 data migration or production release.
...
- The local taxonomy backbone now exists for `statutory-core`, `secure-extension`, and `enrichment-extra`, including runtime/read-model/admin/worker/test coverage. The local runtime import keeps secure-extension words outside statutory Mega/Guardian eligibility.
- Current local spelling runtime counts are `213` statutory-core, `1217` secure-extension, and `33` enrichment-extra words.
- Independent Code Reviewer and Contract Auditor PASS lines are still required before deployment.
