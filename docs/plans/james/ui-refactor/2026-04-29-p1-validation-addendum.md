...
| 6 | **Punctuation inline `#B8873F` (Bellstorm gold) in six JSX call-sites** | Verified via `grep -rn 'B8873F' src/subjects/punctuation/`: hits at `module.js:66` (subject metadata — allowed), `PunctuationSessionScene.jsx:372,525`, `PunctuationMapScene.jsx:354`, `PunctuationSetupScene.jsx:302,328`, `PunctuationSummaryScene.jsx:689`, plus a `:686` comment (excluded). The six inline JSX style sites are the gap; `module.js:66` is the legitimate subject-metadata source. | **U6** — only `PunctuationSetupScene.jsx:302,328` are in P2 scope (introduces `--punctuation-accent`; migrates inline `borderTopColor` / `--btn-accent` to the token). The four remaining JSX sites (`PunctuationSessionScene.jsx:372,525`, `PunctuationMapScene.jsx:354`, `PunctuationSummaryScene.jsx:689`) are deferred per the plan's whole-repo-purge boundary; the U7 token-contract parser test is intentionally scoped to `PunctuationSetupScene.jsx` only to avoid pulling them in. |
