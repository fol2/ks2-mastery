...
- **Navigation vs. mutation is a new concept (`composeIsNavigationDisabled`)**: not a rename. `composeIsDisabled` keeps its exact current behaviour for mutation controls (unchanged across every existing call site). The navigation helper is strictly less-disabled — it flips only when the runtime is truly gone (`unavailable`), not when a command is pending. Rationale: minimises blast radius; every existing caller of `composeIsDisabled` stays correct; only the 4-5 navigation call sites opt in.
...
**Approach:**
- `composeIsNavigationDisabled(ui)` returns `true` only when `ui.availability === 'unavailable'` or `ui.runtime === 'readonly'` (final-lockdown states). `pendingCommand` does not disable navigation. `availability === 'degraded'` does not disable navigation.
...
- Happy path: `ui.availability = 'unavailable'`. Both helpers return `true`.
- Happy path: `ui.runtime = 'readonly'`. Both helpers return `true`.
- Edge case: `ui` is `null` / `undefined`. Both helpers return a safe default (likely `true`, fail-closed).
