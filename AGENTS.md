...
- Cloudflare Wrangler remote-build hosts should export `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1` before the build command runs. `@playwright/test` is a devDependency added in U5 for the golden-path suite; the deployed Worker never loads the Playwright browser, so skipping the ~300 MB Chromium download is safe and faster. See `docs/operations/capacity.md#playwright-test-suite`.
