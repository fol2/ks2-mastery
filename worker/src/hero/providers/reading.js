...
export function readingProvider(readModel) {
  ...
  const signals = {
    ...
    megaLike: overview.securedSkills >= 8,
    postMegaAvailable: overview.securedSkills >= 8,
  ...
  if (signals.weakCount > 0) {
    envelopes.push(buildTaskEnvelope({
      ...
      reasonTags: ['weak-reading-domain', 'evidence-repair'],
      debugReason: `Reading has ${signals.weakCount} weak domain(s) needing repair.`,
    }));
  }
  if (signals.dueCount > 0) {
    envelopes.push(buildTaskEnvelope({
      ...
      reasonTags: ['due-reading-review'],
      debugReason: `Reading has ${signals.dueCount} due review signal(s).`,
    }));
  }
  if (signals.secureCount >= 3) {
    envelopes.push(buildTaskEnvelope({
      subjectId,
      intent: 'retention-after-secure',
      launcher: 'guardian-check',
      effortTarget: 6,
      reasonTags: ['secure-reading-maintenance'],
      debugReason: `Reading has ${signals.secureCount} secured domain(s); maintenance check eligible.`,
    }));
  }
  if (overview.totalQuestions >= 12) {
    envelopes.push(buildTaskEnvelope({
      ...
      launcher: 'mini-test',
      effortTarget: 8,
      reasonTags: ['reading-breadth', 'mixed-texts'],
      debugReason: 'Reading has enough history for a mixed SATs-style mini paper.',
    }));
  }
  if (envelopes.length === 0) {
    envelopes.push(buildTaskEnvelope({
      ...
      launcher: 'smart-practice',
      effortTarget: 6,
      reasonTags: ['reading-baseline'],
      debugReason: 'Reading needs an initial baseline across KS2 domains.',
    }));
  }
