...
import { MONSTER_ASSET_VERSION } from '../src/platform/game/monsters.js';
...
test('monster visual renderers use published config for meadow and reward toast', async () => {
  ...
  assert.ok(html.includes(`src="./assets/monsters/vellhorn/b1/vellhorn-b1-3.1280.webp?v=${MONSTER_ASSET_VERSION}"`));
  ...
  assert.ok(html.includes(`src="./assets/monsters/phaeton/b1/phaeton-b1-4.1280.webp?v=${MONSTER_ASSET_VERSION}"`));
  assert.match(html, /sizes="\(max-width: 820px\) 76vw, 700px"/);
  assert.ok(html.includes(`src="./assets/monsters/glimmerbug/b1/glimmerbug-b1-4.1280.webp?v=${MONSTER_ASSET_VERSION}"`));
