// U6 — Seeded adaptive-selection simulation suite (Part A1-1: Principle 1 first half).
//
...
// -----------------------------------------------------------------------------
// Principle 1 — Due outranks non-due (first half of concepts).
// -----------------------------------------------------------------------------

test('U6 principle: due concept picked more often than non-due (first half of concepts across 8 seeds)', async () => {
  ...
  const concepts = GRAMMAR_CONCEPTS.slice(0, half);
  let dueTotal = 0;
  let notDueTotal = 0;
  const perConcept = [];
  ...
  assert.ok(
    ...
    `Due concepts must be picked more often (first half). `
