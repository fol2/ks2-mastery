...
describe('Punctuation smoke attestation metadata', () => {
  it('smoke at P20 depth asserts 15072 runtime items with correct release ID', () => {
    const attestation = buildAttestationMetadata({ environment: 'local' });
    assert.equal(attestation.runtimeItemCount, 15072);
  ...
  it('runtime count mismatch triggers descriptive error', () => {
    ...
    assert.throws(
      ...
      (error) => {
        ...
        assert.match(error.message, /expected 15072/);
