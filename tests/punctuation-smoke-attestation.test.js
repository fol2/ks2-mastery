...
describe('Punctuation smoke attestation metadata', () => {
  it('smoke at P14 depth asserts 3564 runtime items with correct release ID', () => {
    const attestation = buildAttestationMetadata({ environment: 'local' });
    assert.equal(attestation.runtimeItemCount, 3564);
  ...
  it('runtime count mismatch triggers descriptive error', () => {
    ...
    assert.throws(
      ...
      (error) => {
        ...
        assert.match(error.message, /expected 3564/);
