...
describe('Grammar QG P14 combined freshness and simulation cross-check (part B3-3)', () => {
  it('committed report first half of profiles have no mastery-inflation via shallow items', () => {
    const committed = readReport('grammar-qg-p14-star-pacing-simulation.json');
    const half = Math.ceil(committed.profiles.length / 2);
    for (const profile of committed.profiles.slice(0, half)) {
      assert.equal(
