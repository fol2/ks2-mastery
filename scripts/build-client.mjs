...
const generatedBuildVersionPath = path.join(rootDir, 'worker', 'src', 'generated-build-version.js');
...
export async function runBuildClient({ buildHash } = {}) {
  ...
  await writeGeneratedBuildVersion({ buildHash: resolvedHash });

  return { buildHash: resolvedHash };
}

export async function writeGeneratedBuildVersion({ buildHash, generatedAt = new Date().toISOString() } = {}) {
  const payload = {
    buildHash: buildHash || null,
    source: buildHash ? 'git.rev-parse' : 'unavailable',
    generatedAt,
  };
  const generatedModule = [
    ...
    '',
    `export const BUILD_VERSION = Object.freeze(${JSON.stringify(payload, null, 2)});`,
    '',
  ].join('\n');
  await writeFile(generatedBuildVersionPath, generatedModule, 'utf8');
}
